...
function InfinityArrowLogo() {
  return (
    <svg viewBox="0 0 120 70" className="h-12 w-auto md:h-14" aria-hidden="true">
      <defs>
    ...
    <header className="sticky top-0 z-50 border-b border-navy/10 bg-cream/95 backdrop-blur">
      <div className="mx-auto max-w-7xl px-6 py-3 md:px-10">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <Link href="/" className="flex items-center gap-3" aria-label="Scelta Infinity home">
            <InfinityArrowLogo />
            <span className="font-serif text-2xl font-bold text-gold md:text-3xl">{company.brandName}</span>
          </Link>
        ...
        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-navy/10 pt-2 text-xs text-navy/80">
          {contactLinks.map((link) => (
            <a key={link.label} href={link.href} target={link.href.startsWith("http") ? "_blank" : undefined} rel={link.href.startsWith("http") ? "noreferrer" : undefined} className="hover:text-gold">
